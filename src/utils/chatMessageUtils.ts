 ...
 */
export function extractVoiceMessagesInfo(logs: StreamingLog[]): Map<number, VoiceMessageInfo> {
...
export function processClientContentMessage(
  ...
  const closestVoiceTimestamp = [...voiceMessagesInfo.keys()]
    ...
    .sort((a, b) => Math.abs(a - messageTimestamp) - Math.abs(b - messageTimestamp))[0];

  if (closestVoiceTimestamp !== undefined && Math.abs(closestVoiceTimestamp - messageTimestamp) < 2000) {
    isVoiceMessage = true;
...
export function processUnmatchedVoiceMessages(
  ...
  return [...voiceMessagesInfo.entries()].filter(([, info]) => !info.matched).sort(([tsA], [tsB]) => tsA - tsB);
}
...
export function integrateMessages(
  ...
  const sortedMessages = [...chatMessages].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());

  while (messageIndex < sortedMessages.length || voiceInfoIndex < unmatchedVoiceInfos.length) {
    const currentMessageTimestamp =
      messageIndex < sortedMessages.length ? sortedMessages[messageIndex].timestamp.getTime() : Infinity;
