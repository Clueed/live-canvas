...
const formatTime = (d: Date) => d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }); // Simplified format
...
const LogEntry = ({
    ...
    const direction = log.type.includes('receive') ? 'receive' : log.type.includes('send') ? 'send' : '';
    ...
    return (
        <li
            ...
            <div className='flex justify-between'>
                <span className='w-40 flex-shrink-0 truncate font-medium' title={log.type}>
                    {log.type}
                </span>
            ...
            {log.count && (
                <span className='ml-2 rounded bg-gray-200 px-1.5 py-0.5 text-xs dark:bg-gray-700'>{log.count}</span>
            )}
        </li>
    );
};

const PlainTextMessage = ({ message }: { message: StreamingLog['message'] }) => <span>{message as string}</span>;
...
const RenderPart = ({ part }: { part: Part }) => {
    if (part.text && part.text.trim().length) {
        return <p className='part part-text my-1 whitespace-pre-wrap'>{part.text}</p>;
    }
    ...
    if (part.inlineData) {
        return (
            <div className='part part-inlinedata my-1 text-gray-500'>
                <h5 className='text-xs italic'>Inline Data: {part.inlineData.mimeType}</h5>
            </div>
        );
    }
...
const ClientContentLog = ({ message }: Message) => {
    ...
    return (
        <div className='rich-log client-content user my-1 rounded border border-green-300 bg-green-50 p-2 dark:border-green-700 dark:bg-green-900/30'>
            <h4 className='mb-1 text-xs font-semibold text-green-800 dark:text-green-200'>User</h4>
            {turns.map((turn, i) => (
            ...
            {!turnComplete && <span className='mt-1 block text-xs text-gray-500 italic'>turnComplete: false</span>}
        </div>
    );
};
...
const ToolCallCancellationLog = (
    ...
    <div className='rich-log tool-call-cancellation my-1 rounded border border-red-300 bg-red-50 p-2 text-red-700 dark:border-red-700 dark:bg-red-900/30 dark:text-red-300'>
        ...
        {(message as ToolCallCancellationMessage).toolCallCancellation.ids.map((id, index) => (
            <React.Fragment key={`cancel-${id}`}>
                <code className='mx-1 rounded bg-red-100 px-1 py-0.5 text-xs dark:bg-red-800'>{id}</code>
                {index < (message as ToolCallCancellationMessage).toolCallCancellation.ids.length - 1 ? ', ' : ''}
            </React.Fragment>
        ))}
    </div>
);
...
const ToolResponseLog = (
    ...
    <div className='rich-log tool-response my-1 rounded border border-cyan-300 bg-cyan-50 p-2 dark:border-cyan-700 dark:bg-cyan-900/30'>
        {(message as ToolResponseMessage).toolResponse.functionResponses.map((fr) => (
...
const ModelTurnLog = ({ message }: Message) => {
    ...
    return (
        <div className='rich-log model-turn model my-1 rounded border border-blue-300 bg-blue-50 p-2 dark:border-blue-700 dark:bg-blue-900/30'>
            <h4 className='mb-1 text-xs font-semibold text-blue-800 dark:text-blue-200'>Model</h4>
            {parts
                .filter((part) => !(part.text && part.text.trim() === '')) // Filter empty text parts
                .map((part, j) => (
...
const filters: Record<LoggerFilterType, (log: StreamingLog) => boolean> = {
    ...
    conversations: (log: StreamingLog) => isClientContentMessage(log.message) || isServerContentMessage(log.message),
    none: () => true
};

const getMessageComponent = (log: StreamingLog): (({ message }: Message) => ReactNode) => {
...
export function Logger({ filter = 'none' }: LoggerProps) {
    ...
    const filteredLogs = React.useMemo(() => logs.filter(filterFn), [logs, filterFn]);
        ...
        <div className='max-w-full'>
            <ul className='max-w-full list-none p-0'>
                {filteredLogs.map((log: StreamingLog, index: number) => {
                    ...
                    return <LogEntry MessageComponent={MessageComponent} log={log} key={index} />;
                })}
            </ul>
        </div>
    );
