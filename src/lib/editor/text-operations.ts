...
export const setEditorArtifactOperation = {
  /**
   * Zod schema for set_editor_artifact parameters
   */
...
Does *not* merge with existing text; always supplies a full artifact string.
  `.trim(),
    // Manually defining the schema to match the zod schema for the FunctionDeclaration
    parameters: {
      type: SchemaType.OBJECT,
      properties: {
        text: {
   ...
   */
  create: function (editor: PlateEditor) {
    return function setEditorArtifact(args: z.infer<typeof setEditorArtifactOperation.paramsSchema>) {
