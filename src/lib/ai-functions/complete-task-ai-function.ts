...
export const completeTaskOperation = defineAiFunction({
  declaration: {
    name: "complete_task",
    description:
      "Signals the definitive conclusion of the current request or assigned task. This function marks the final step in the workflow and MUST be called exactly once to finalize a task.\n\n**When to Call:**\n* **Single-Turn Completion:** If the task can be fully addressed by initiating one or more function calls within the current turn, and no subsequent steps *depend* on the results of these calls to continue the *same* task, you SHOULD include `complete_task` alongside the other function call(s) in this turn.\n* **Assumption of Success:** When calling `complete_task` concurrently with other function calls, assume those other calls will succeed. If any of them fail, the system will prevent the task from being marked as complete and will inform you of the error, allowing you to handle it in the next turn. You do not need to anticipate errors.\n* **Multi-Turn Completion:** If the task requires multiple steps across turns (e.g., you must wait for the result of function call A before deciding on or performing action B), defer calling `complete_task` until the very final action of the task is performed in a subsequent turn.\n\nIn summary, call `complete_task` as soon as all planned actions for the current task have been initiated, unless you explicitly need to wait for results from those actions to determine further steps *within that same task*.",
    parameters: {
